...
int correct(string &str){
	int x = str.size() - 1;
	if(str.find('.') != -1) while(x > 0){
		if(str[x] == '0') x--;
		else break;
	}
	str = str.substr(0, x + 1);
	error(str.back());
	if(str.back() == '.') str = str.substr(0, sz(str) - 1);
	if(str[0] == '0' && str.size() > 1 && str[1] != '.') str = str.substr(1); 
	cout << str << endl;
}
...
int main(){
	...
	a = str.substr(0, 1);
	int pos = str.find('e');
	b = str.substr(2, pos - 2);
	c = str.substr(pos + 1);

	if(b.size() <= stoi(c)){
		string str = a + b + string(stoi(c) - (int)b.size(), '0');
		correct(str);
	}
	else{
		string str = a + b.substr(0, stoi(c)) + string(1, '.') + b.substr(stoi(c)) ;
		correct(str);
	}
