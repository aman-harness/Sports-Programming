...
int main(){
	...
	R(i, 27){
		...
		// cout << i _ mark[str[i] - 'A'] << endl;
	}

	// mark[pos] , pos
	if(pos - mark[str[pos] - 'A'] == 1){
		cout << "Impossible\n";
		return 0;
	}
	else{
		// Error(pos, mark[str[pos] - 'A']);
		if(mark[str[pos] - 'A'] <= 13){
		str.erase(pos, 1); str += str;				
		cout << str << endl;
		}
		int x = pos - mark[str[pos] - 'A']; x /= 2;
		for(int i = 0; i < 13; i++){
			a[i] = (str[(39 + mark[str[pos - 'A']] + x + i) % 26]);
		}
	}
	cout << a << endl;
	return 0;
