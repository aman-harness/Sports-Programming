...
int main(){
	...
	for(int i = 0; i < n; i++){
		...
		ll c = 1, temp = 0;
		int p = 0; 
		while(c * inp[i] <= inp[n - 1]){
			if(c * inp[i] < (ll)N) A[c * inp[i]] = 1;
			//int p = lower_bound(inp + i, inp + n, c * inp[i]) - (inp + i);
			int p2 = lower_bound(inp + i, inp + n, (c + 1) * inp[i]) - (inp + i);
			temp += (p2 - p) * inp[i] * c++;
			p = p2;
		}
		//Error(inp[i], temp);
